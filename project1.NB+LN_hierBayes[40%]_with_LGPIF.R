########### Bayes Parametric: Hierarchical LN-GLM [BASIC] with NDB ############# see 1095 for zeta
...
##### LGPIF Data with overall error: [40%]-[40%]-[40%]-[40%]-[40%]-[40%]-[40%]-[40%]-[40%]
LGPIF <- read.csv("C:/Users/kimm4/Desktop/WORKSPACE/2nd 3rd paper/DATASET/LGPIF.paper3-40.csv")
head(LGPIF)
...
lppd_EX_S.Y.xx = mean( colSums(loglikelihood.xx) ); lppd_EX_S.Y.xx #-18058.41
...
#1        2        3        4        5        6 
#3.491788 3.451738 3.641833 3.879226 3.879226 3.618810 
...
#summary(fit1S.st)
fit1S.st <- glm( Y ~ XSstar + factor(ZS), family=poisson(link="log"))
...
# -16870.89 (0.7), -16816.78 (0.8), -16785.82 (0.9)
...
for(r in 1:(total_iter-r_convergence)) {
  ...
  for(i in 1:n.train) {
    ...
    expvalS.train.st[i,r] <- exp( betaparamcleanS[j,1] + betaparamcleanS[j,2]*dirtyXS + 
                                    betaparamcleanS[j,3]*ZS[i] - 0.5*sig2paramclean[j]-0.5)
...
# test.dirty = EX_S.train.dirty
# test.st = EX_S.train.st.Large
...
for(j in 1:JS) {
  ...
  lines(density(log(test.dirty)[cl_membershipS==j]), col="black", lty = "dotted") 
  #lines(density(log(test.st)[cl_membershipS==j]+0.1), col="blue", lwd=1) 
...
EX_S.test.Y.xx
SSPE.LN.xx <- sum( (log(EX_S.test.Y.xx) - log(test.df$yAvg))^2 ); SSPE.LN.xx   #861.4435
SAPE.LN.xx <- sum( abs(log(EX_S.test.Y.xx) - log(test.df$yAvg)) ); SAPE.LN.xx  #437.7337  
...
group_params.st <- list(
  list(xi_F=mean(EX_F.train[cl_membershipF==1]), psi_F=avg_psi.vec1, 
       meanlog_S=mean(mu.vec.st.train[cl_membershipF==1]), 
       sdlog_S=sqrt(avg_sig2.vec1.st), 
       n=table(cl_membershipF)[1]),
  list(xi_F=mean(EX_F.train[cl_membershipF==2]), psi_F=avg_psi.vec2, 
       meanlog_S=mean(mu.vec.st.train[cl_membershipF==2]), 
       sdlog_S=sqrt(avg_sig2.vec2.st), 
       n=table(cl_membershipF)[2]),
  list(xi_F=mean(EX_F.train[cl_membershipF==3]), psi_F=avg_psi.vec3, 
       meanlog_S=mean(mu.vec.st.train[cl_membershipF==3]), 
       sdlog_S=sqrt(avg_sig2.vec3.st), 
       n=table(cl_membershipF)[3]),
  list(xi_F=mean(EX_F.train[cl_membershipF==4]), psi_F=avg_psi.vec4, 
       meanlog_S=mean(mu.vec.st.train[cl_membershipF==4]), 
       sdlog_S=sqrt(avg_sig2.vec4.st), 
       n=table(cl_membershipF)[4]),
  list(xi_F=mean(EX_F.train[cl_membershipF==5]), psi_F=avg_psi.vec5, 
       meanlog_S=mean(mu.vec.st.train[cl_membershipF==5]), 
       sdlog_S=sqrt(avg_sig2.vec5.st), 
       n=table(cl_membershipF)[5]),
  list(xi_F=mean(EX_F.train[cl_membershipF==6]), psi_F=avg_psi.vec6, 
       meanlog_S=mean(mu.vec.st.train[cl_membershipF==6]), 
...
lppd_EX_S.Y   #-16155.9
lppd_EX_S.Y.xx#-18058.41
lppd_EX_S.Y.st#-16770.85
...
SSPE.LN.xx <- sum( (log(EX_S.test.Y.xx) - log(test.df$yAvg))^2 ); SSPE.LN.xx   #861.4435
SAPE.LN.xx <- sum( abs(log(EX_S.test.Y.xx) - log(test.df$yAvg)) ); SAPE.LN.xx  #437.7337  
SSPE.LN.st <- sum( (log(EX_S.test.Y.st) - log(test.df$yAvg))^2 ); SSPE.LN.st   #863.5006
SAPE.LN.st <- sum( abs(log(EX_S.test.Y.st) - log(test.df$yAvg)) ); SAPE.LN.st  #438.3646
SSPE.simex <- sum( (log(pred.val.Y.simex) - log(test.df$y))^2 ); SSPE.simex   #1954.039
SAPE.simex <- sum( abs(log(pred.val.Y.simex) - log(test.df$y)) ); SAPE.simex  #732.7267
